...
import static TestUtils.RandomTestData.*;
...
//    int randomNumber = Math.abs(random.nextInt()); //Math.abs() — это метод, который возвращает не отрицательное число (а в параметре метода указано получение рандомного числа)
    ...
    @BeforeEach //Указали данную переменную, чтобы перед каждым тестом была инициализацию юзера
    public void initTestUser() {
        user = getRandomUserWithGames(); //Инициализировали рандомного юзера с играми, чтобы каждый раз не создавать его в методах
...
//    }
    ...
    @Test
    public void positiveRegisterWithGamesTest() { //создали данный метод, чтобы протестировать регистрацию
//        int randomNumber = Math.abs(random.nextInt()); //Math.abs() — это метод, который возвращает не отрицательное число (а в параметре метода указано получение рандомного числа)
        FullUser user = getRandomUserWithGames(); //собрали экземпляр объекта FullUser с вызовом метода getRandomUserWithGames (в данном экземпляре указаны данные с помощью faker)
        Response response = userService.register(user) //вызвали метод register для создания пользователя
        ...
        SoftAssertions softAssertions = new SoftAssertions();// Создали экземпляр библиотеки SoftAssertions, чтобы использовать её методы

        softAssertions.assertThat(info.getMessage()).as("Сообщение об ошибке было не верное")
                .isEqualTo("Фэйковое сообщение");

        softAssertions.assertThat(response.statusCode()).as("Статус код не был 200")
                .isEqualTo(200); //Создали ассерт на статус код 200 - если этот момент завалится, то тест не упадёт и продолжит прогон
    ...
    public void positiveRegisterTestWithRandom() { //создали данный метод, чтобы протестировать регистрацию
        userService.register(user)
                .should(hasStatusCode(201)) //проверка на статус код
                .should(hasMessage("User created")); //проверка на сообщение о создании польз-ля
...
//        Assertions.assertEquals("User created", info.getMessage()); //В параметрах мы указали, что ожидаем сообщение "User created" и сравнили его с getMessage при отправке запроса
    ...
    @Test
    public void PositiveAuthAdminTest (@AdminUser FullUser admin) {
